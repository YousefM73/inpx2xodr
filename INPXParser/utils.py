"""Utility functions for coordinate transformations and geometry calculations."""

__all__ = ['crs_4326', 'crs_3395', 'transformer', 'giveHeading', 'checkDistance', 'drehen', 'rotateToXAxis',
...
topomap = None
topoParameter = None
def convertTopoMap(topomappath, datapath=None):
    global topomap
    global topoParameter
    if topomappath is not None:
        ...
        topomap=np.rot90(topomap)
    topoParameter = giveMaxMinLongLat(datapath)
...
def giveMaxMinLongLat(osmpath=None, trustOSMHeaderMinMax=False):
    """
    Placeholder function for coordinate bounds.
    For INPX files, bounds are calculated during parsing.
    """
    global referenceLat
    global referenceLon
    global transformer
    
    # Default bounds - these will be overridden by actual data during parsing
    minlat = 0.0
    maxlat = 0.001  
    minlon = 0.0
    maxlon = 0.001
...
def convertLongitudeLatitude(longitude, latitude):
    global transformer, referenceLat, referenceLon
    ...
    if transformer is None:
        ...
        # Initialize the projectionTransformer
        uproj = CRS.from_proj4("+proj=tmerc +lat_0={0} +lon_0={1} +x_0=0 +y_0=0 +ellps=GRS80 +units=m".format(referenceLat, referenceLon))
        transformer = Transformer.from_crs(crs_4326, uproj)
...
def giveReferences():
    ...
    # Ensure we have default values if not initialized
    if referenceLat is None:
        referenceLat = 0.0
    if referenceLon is None:
        referenceLon = 0.0
    if topoParameter is None:
        # Calculate default bounds based on current coordinate system
        topoParameter = giveMaxMinLongLat()
